  ...
  return (
    ...
    >
      <div className="p-5 flex gap-4 items-center">
        <div
          className={["w-28 h-[72px] rounded-2xl overflow-hidden border", selected ? "border-white/20" : "border-white/10"].join(
            " "
          )}
        >
          <ShapeArt id={id} />
...
const Step05FingerTest = (() => {
  ...
  return function Step05FingerTest({
    onNext,
    onBack,
    toast
  }: {
    ...
    return (
      <div className="w-full max-w-md mx-auto flex flex-col min-h-0 flex-1 px-6 pt-8 pb-10">
        ...
        <div className="mt-7 grid grid-cols-1 gap-4">
          {options.map((o) => (
            <Card key={o.gap} selected={selected === o.gap} onClick={() => pick(o.gap)} title={o.title} sub={o.sub} />
          ))}
        </div>
...
const Step06TissueDepth = (() => {
  ...
  return function Step06TissueDepth({
    onNext,
    onBack,
    toast
  }: {
    ...
    return (
      <div className="w-full max-w-md mx-auto flex flex-col min-h-0 flex-1 px-6 pt-8 pb-10">
        ...
        <div className="mt-7 flex flex-col gap-4">
          {opts.map((o) => (
            <Card key={o.id} selected={selected === o.id} onClick={() => pick(o.id)} title={o.title} sub={o.sub} icon={o.icon} />
          ))}
        </div>
...
const Step07SabotageCheck = (() => {
  ...
  return function Step07SabotageCheck({
    onNext,
    onBack,
    toast
  }: {
    ...
    return (
      <div className="w-full max-w-md mx-auto flex flex-col min-h-0 flex-1 px-6 pt-8 pb-10 relative">
        ...
        <div className="mt-7 grid grid-cols-1 gap-3">
          {OPTIONS.map((o) => (
            <Chip key={o.id} selected={selected.includes(o.id)} onClick={() => toggle(o.id)} label={o.label} redFlag={o.redFlag} />
          ))}
        </div>
...
const Step08Symptoms = (() => {
  ...
  return function Step08Symptoms({
    onNext,
    onBack,
    toast
  }: {
    ...
    return (
      <div className="w-full max-w-md mx-auto flex flex-col min-h-0 flex-1 px-6 pt-8 pb-10">
        ...
        <div className="mt-7 flex flex-col gap-3">
          {OPTIONS.map((o) => (
            <Row key={o.id} selected={selected.includes(o.id)} onClick={() => toggle(o.id)} label={o.label} icon={o.icon} />
          ))}
        </div>
...
const Step09Timeline = (() => {
  ...
  return function Step09Timeline({
    onNext,
    onBack,
    toast
  }: {
...
const Step11Commitment = (() => {
  ...
  return function Step11Commitment({
    onNext,
    onBack,
    toast
  }: {
...
const Step12Analysis = (() => {
  ...
  return function Step12Analysis({ onDone }: { onDone: () => void }) {
    const name = useUserStore((s) => s.name) || "there";
...
const Step13PlanReveal = (() => {
  ...
  return function Step13PlanReveal({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
    const name = useUserStore((s) => s.name) || "there";
    ...
    const insights = useMemo(() => {
      ...
      list.push(
        `Your daily commitment: ${commitment === "5-7" ? "5 Minutes" : commitment === "15" ? "15 Minutes" : "30 Minutes"}.`
      );
      return list;
  ...
  const CheckoutForm = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
      ...
      >
        ...
        {message && (
          <div className="text-red-300 text-sm mt-4 bg-red-500/10 p-3 rounded-xl border border-red-500/20 font-semibold">{message}</div>
        )}
      ...
      <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={onClose}>
        <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm bg-[#1A1A26] border border-white/10 rounded-[32px] p-6 shadow-2xl">
          ...
          <form onSubmit={handleRestoreSubmit} className="flex flex-col gap-4">
            ...
            {message && (
              <div className="text-red-300 text-sm bg-red-500/10 p-3 rounded-xl border border-red-500/20 font-semibold">{message}</div>
            )}
...
// =====================================
// ✅ FINAL: Onboarding Wrapper (Default)
// =====================================
