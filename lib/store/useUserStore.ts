...
export type UserState = {
  ...
  checkins: Record<string, boolean>; // `${cycleKey}:${milestoneDay}`
...
export const useUserStore = create<UserState>()(
  persist(
    (set, get) => ({
      ...
      setPremium: (val) => {
        // when unlocking premium, reset bridge so they see the Protocol Generator
      ...
      addPainLog: (entry) =>
        set((s) => ({ painLogs: [entry, ...s.painLogs].slice(0, 200) })),
